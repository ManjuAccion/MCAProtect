...
class MCAChatBubble: UIView {
    ...
    /**
     ...
     :returns: Chat Bubble
     */
    init(data: MCAChatBubbleData, startY: CGFloat){
        
        // 1. Initializing parent view with calculated frame
        super.init(frame: MCAChatBubble.framePrimary(data.type, startY:startY))
        ...
        if data.type == .mine {
            ...
            let extraWidthToConsider = imageViewBG!.frame.width + 45.0
            ...
            newStartX = -imageViewBG!.frame.minX + 3.0 + 45.0
        }
